...
// 5. Replace Invalid Tags
content = content.replace(/clipRule="evenodd"\s*\/\>\<\/path\>\<\/svg\>/g, `clipRule="evenodd" /></svg>`);

// 6. UI Updates (Buttons)
const oldButtonsRegex = /<button[\s\S]*?onClick=\{\(\) => setShowDesigner\(true\)\}[\s\S]*?iWindoor 2D Çizim[\s\S]*?<\/button>[\s\S]*?<div className="w-px h-8 bg-slate-700 hidden md:block"><\/div>[\s\S]*?<input[\s\S]*?type="file"[\s\S]*?accept=".xlsx, .xls"[\s\S]*?className="hidden"[\s\S]*?ref=\{fileInputRef\}[\s\S]*?onChange=\{handleFileUpload\}[\s\S]*?\/>[\s\S]*?<button[\s\S]*?onClick=\{\(\) => fileInputRef\.current\?\.click\(\)\}[\s\S]*?Excel Yükle[\s\S]*?<\/button>[\s\S]*?<button[\s\S]*?Dýþa Aktar[\s\S]*?<\/button>[\s\S]*?<button[\s\S]*?PDF[\s\S]*?<\/button>/m;
...
if (oldButtonsRegex.test(content)) {
    ...
    console.log("Could not find the buttons to replace.");
}

// 7. Add Modal before WindowDesigner
const modal = `
      {/* UPGRADE MODAL */}
      {showUpgradeModal && (
...
console.log("Applied UI updates successfully via Node.");
