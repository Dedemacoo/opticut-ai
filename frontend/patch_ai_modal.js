...
// 1. Add state for AI Modal
const stateBlock = `  const [showUpgradeModal, setShowUpgradeModal] = useState(false);
  const [showAiModal, setShowAiModal] = useState(false);
  const [aiLoading, setAiLoading] = useState(false);
  const [aiImage, setAiImage] = useState<File | null>(null);
  const [aiPreview, setAiPreview] = useState<string | null>(null);`;

content = content.replace("  const [showUpgradeModal, setShowUpgradeModal] = useState(false);", stateBlock);

// 2. Change the button onClick to open AI Modal
const buttonActionOld = `onClick={() => handleFeatureClick("AI Fotoğraf Yükleme ve OptiCut Copilot", "Pro Plus", () => { alert("AI Yükleme Modülü Açılıyor (Mock)") })}`;
const buttonActionNew = `onClick={() => handleFeatureClick("AI Fotoğraf Yükleme ve OptiCut Copilot", "Pro Plus", () => setShowAiModal(true))}`;
content = content.replace(buttonActionOld, buttonActionNew);

// 3. Insert the AI Modal JSX right after the UpgradeModal
const upgradeModalEnd = `          </div>
        </div>
      )}`;
...
content = content.replace(upgradeModalEnd, upgradeModalEnd + aiModalBlock);
