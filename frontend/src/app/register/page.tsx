...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Basarili kayittan sonra login"e veya direkt iceri alalim (su an direkt iceri)
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 flex flex-col justify-center items-center p-4 relative overflow-hidden">
      <div className="absolute top-[10%] right-[-10%] w-[40%] h-[40%] bg-blue-600/20 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">Yeni Hesap Olusturun</h1>
          <p className="text-slate-400 text-sm mt-2">14 Gunluk Ucretsiz Deneme ile Baslayin</p>
        </div>

        <form onSubmit={handleRegister} className="bg-slate-900/40 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-slate-700/50">
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-widest">Ad Soyad</label>
              <input 
                type="text" 
                required
                value={name}
                ...
                placeholder="Orn: Ugur Demir"
              />
            </div>
            
            <div>
              <label className="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-widest">Firma Adi</label>
              <input 
                type="text" 
                value={company}
                onChange={(e) => setCompany(e.target.value)}
                className="w-full p-3.5 bg-slate-950/50 border border-slate-700/50 rounded-xl text-slate-200 focus:ring-2 focus:ring-blue-500/50 outline-none transition-all"
                placeholder="Firmaniz LTD STI."
              />
            </div>
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 px-4 rounded-xl shadow-[0_0_15px_rgba(37,99,235,0.3)] transition-all flex justify-center items-center mt-4 disabled:opacity-70"
            >
              {isLoading ? (
        ...
        <p className="text-center text-sm text-slate-400 mt-8 font-medium">
          Zaten hesabiniz var mi? <Link href="/login" className="text-blue-400 hover:text-blue-300 font-bold ml-1">Giris Yapin</Link>
        </p>
      </div>
    </div>
  );
