...
export default function SettingsPage() {
  ...
  useEffect(() => {
    // Component yuklendiginde local storage"dan kullanici verilerini al
    ...
    if (storedUserStr) {
      try {
        ...
        if (typeof storedUser === "object") {
          setUser(storedUser);
          if (storedUser.plan) setPlan(storedUser.plan as PlanType);
        }
      } catch (e) {
        console.error("Kullanici bilgileri alinamadi.");
      }
    }
  }, []);
  ...
  const planDetails: Record<PlanType, { price: string, desc: string, icon: any, color: string }> = {
    "Standart": {
      price: "3.500 ₺",
      desc: "Sadece Fire ve Kesim Hesabi",
    ...
    "Pro": {
      price: "7.000 ₺",
      desc: "iWindoor Cizim Ozelligi Aktif",
    ...
    "Pro Plus": {
      price: "15.000 ₺",
      desc: "Tum Ozellikler + Yapay Zeka Asistani Sinirsiz",
    ...
    <div className="animate-in fade-in duration-500 max-w-5xl mx-auto">
      <div className="mb-8 border-b border-slate-800 pb-5 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1.5">Uygulama tercihlerinizi, abonelik ve kurumsal bilgilerinizi yonetin.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="bg-[#1e293b] rounded-2xl border border-slate-700/50 overflow-hidden shadow-lg sticky top-6">
            <div className="p-4 border-b border-slate-800 bg-slate-800/30">
              <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Menu</h3>
            </div>
            <div className="flex flex-col p-2 space-y-1">
              <button 
                onClick={() => setActiveTab("account")}
                className={`px-4 py-2.5 text-sm text-left rounded-xl transition-all flex items-center gap-3 ${activeTab === "account" ? "bg-blue-600/10 text-blue-400 font-semibold" : "text-slate-400 hover:bg-slate-800 hover:text-slate-200"}`}
              >
              ...
              <button 
                ...
                className={`px-4 py-2.5 text-sm text-left rounded-xl transition-all flex items-center gap-3 ${activeTab === "support" ? "bg-blue-600/10 text-blue-400 font-semibold" : "text-slate-400 hover:bg-slate-800 hover:text-slate-200"}`}
              >
        ...
        <div className="flex-1">
          ...
          {/* HESAP BILGILERIM */}
          {activeTab === "account" && (
            <div className="bg-[#1e293b] p-7 rounded-2xl border border-slate-700/50 shadow-xl animate-in fade-in slide-in-from-bottom-2">
              <h3 className="text-sm font-bold text-slate-100 mb-6 flex items-center gap-2">
                <span className="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></span>
                Kisisel Hesap Bilgileriniz
              </h3>
              ...
              {user ? (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Ad Soyad</label>
                      <input type="text" readOnly value={user.name} className="w-full p-3 bg-slate-900/50 border border-slate-700 rounded-xl text-sm text-slate-300 outline-none cursor-not-allowed opacity-80" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">E-Posta Adresi</label>
                      <input type="email" readOnly value={user.email} className="w-full p-3 bg-slate-900/50 border border-slate-700 rounded-xl text-sm text-slate-300 outline-none cursor-not-allowed opacity-80" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Firma Adi (Kayitli)</label>
                      <input type="text" readOnly value={user.company || "Belirtilmemis"} className="w-full p-3 bg-slate-900/50 border border-slate-700 rounded-xl text-sm text-slate-300 outline-none cursor-not-allowed opacity-80" />
                    </div>
                  </div>
                  
                  <div className="pt-6 border-t border-slate-800">
                    <h4 className="text-sm font-semibold text-slate-200 mb-4">Guvenlik</h4>
                    <button 
                      onClick={handleResetPassword}
                      className="bg-slate-800 hover:bg-slate-700 border border-slate-600 text-white px-5 py-2.5 rounded-lg text-sm transition-all"
                    >
                      Sifremi Sifirla (E-posta Gonder)
                    </button>
                  </div>
                </div>
              ) : (
                <div className="text-slate-400 text-sm">Hesap bilgileri yuklenemiyor...</div>
              )}
            </div>
          )}
              ...
              <div className="bg-[#1e293b] p-7 rounded-2xl border border-slate-700/50 shadow-xl">
                <div className="mb-6">
                  <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
                    ...
                    Varsayilan Optimizasyon Degerleri
                  </h3>
                  <p className="text-xs text-slate-500 mt-2 pl-10">Yeni bir uretim plani olusturuldugunda bu degerler otomatik olarak formda yer alacaktir.</p>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pl-10">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Varsayilan Stok Boyu (mm)</label>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Varsayilan Bicak Payi / Kerf (mm)</label>
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between py-3 border-b border-slate-800">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">Sistem guncellemeleri e-posta ile gelsin.</p>
                    </div>
          ...
          {/* DESTEK VE BILGI (Legal/Support) */}
          {activeTab === "support" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
              <div className="bg-[#1e293b] p-7 rounded-2xl border border-slate-700/50 shadow-xl">
                <h3 className="text-sm font-bold text-slate-100 mb-6 flex items-center gap-2">
                  <span className="w-8 h-8 rounded-lg bg-cyan-500/10 flex items-center justify-center text-cyan-400"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z" /></svg></span>
